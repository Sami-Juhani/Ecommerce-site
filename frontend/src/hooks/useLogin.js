...
const useLogin = () => {
  ...
  const login = async () => {
    try {
      const response = await fetch("https://project-api-gi6r.onrender.com/api/user/login", {
        method: "POST",
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (response) => {
      try {
        ...
        if (res.status === 200) {
          const res2 = await fetch("https://project-api-gi6r.onrender.com/api/user/login", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
